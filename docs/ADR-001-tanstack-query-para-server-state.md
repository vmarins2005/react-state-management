...
| RSC + `fetch` do Next | Zero JS no cliente | Só resolve leitura no servidor; interação continua precisando de cache no cliente | Complementar, não substituto — ver o projeto [react-nextjs-caching](https://github.com/vmarins2005/react-nextjs-caching) |
